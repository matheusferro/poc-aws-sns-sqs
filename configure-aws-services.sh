...
LogInfo "Subscritpion SNS topic 'subscription-topic' on queue 'notification01' and set FilterPolicy"
...
then
  ...
  aws --endpoint-url http://localhost:4566 sns set-subscription-attributes --subscription-arn "$arn_subscription" --attribute-name FilterPolicy --attribute-value "{\"notify_option\":[\"opt1\"]}"
...
if output=$(aws --endpoint-url http://localhost:4566 sns subscribe --topic-arn arn:aws:sns:us-east-1:000000000000:subscription-topic --protocol sqs --notification-endpoint http://localhost:4566/000000000000/notification02 2>&1);
then
  arn_subscription=$(echo $output | awk '{print $3}')
  arn_subscription=${arn_subscription: 1:-1}
  aws --endpoint-url http://localhost:4566 sns set-subscription-attributes --subscription-arn "$arn_subscription" --attribute-name FilterPolicy --attribute-value "{\"notify_option\":[\"opt2\"]}"
  LogSuccess "Subscribed" "$output"
else
  LogError "error on create subscriptions" "$output"
fi